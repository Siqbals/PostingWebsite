...
function Searchresults() {
    ...
    const usernamedecode = location.state.usernamedecode;
    const searchquery = location.state.searchbar;
    ...
    function getsearchresults() {
        nav('/searchresults', {state: { searchbar } })
    }
    ...
    return (
        <div className='background'>
            <div style={{fontSize: '50px', padding: '8px', color: '#429bf5'}}>Search Results</div>

            <div>
                {posts.slice().reverse().filter(post => post.topic.includes(searchquery)).map((post) => (
                    <div key={post.id}>
                        <div style={postbox}>
